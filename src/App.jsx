...
export default function App() {
  ...
  const receivedLikesRaw = notifications.filter(n =>
    (n.type === "profile_like" || n.type === "like") && !matches[n.fromUserId]
  );
  const receivedLikes = [];
  const seenIds = new Set();
  for (const n of receivedLikesRaw) {
    if (!seenIds.has(n.fromUserId)) {
      seenIds.add(n.fromUserId);
      receivedLikes.push(n);
    }
  }
        ...
        <div style={{ flex:1,overflowY:"auto",padding:16 }}>
          {receivedLikes.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 13, color: "#6b8f71", marginBottom: 6 }}>
                あなたに共感している人
              </div>
