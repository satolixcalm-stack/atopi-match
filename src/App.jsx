  ...
  if (screen === "matches") return (
    <div style={S.app}>
      <div style={S.page}>
            ...
            return (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                {hasMore && (
                  ...
                  >
                    もっと見る（あと {allMatches.length - visibleCount} 件）
                  </button>
                )}
                {!hasMore && allMatches.length > 3 && (
                  <div style={{ textAlign: "center", fontSize: 12, color: "#a8c5b0", padding: "10px 0" }}>
                    すべて表示しました
                  </div>
                )}
              </div>
        ...
        <div style={S.nav}>
          <button style={S.navBtn} onClick={() => { setVisibleCount(3); setScreen("browse"); }}>🔍 探す</button>
          <button style={{ ...S.navBtn, color: "#52a875", borderTop: "2px solid #52a875" }}>💚 マッチ</button>
          <button style={S.navBtn} onClick={() => { setVisibleCount(3); setScreen("mypage"); }}>👤 マイページ</button>
        </div>
      </div>
    </div>
  );
...
const S = {
  ...
  loadMoreBtn: { width: "100%", background: "#f0f7f2", color: "#52a875", border: "1.5px solid #c8e6c9", borderRadius: 14, padding: "13px 0", fontSize: 14, fontWeight: 700, cursor: "pointer", marginTop: 4 },
